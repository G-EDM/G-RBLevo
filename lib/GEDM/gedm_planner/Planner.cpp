...
void IRAM_ATTR toggle_pwm( bool turn_off, int delay ){
    ...
    xQueueSend( sensor_queue_main, &data, 50000 );
    while( ( turn_off ? !pconf.pwm_is_off : pconf.pwm_is_off ) ){ 
        if( sys.abort || sys_rt_exec_state.bit.motionCancel || !edm_process_is_running ){ break; }
        delayMicroseconds(10); 
    } 
    delayMicroseconds( delay );*/
...
int IRAM_ATTR G_EDM_PLANNER::get_motion_plan(){
    ...
    if( _state.motion_plan < 0 ){
       ...
       if( _state.motion_plan <= 2 ){
          ...
          ++no_load_steps; // dirty workaround for now. Case counts as load but sometimes we just switch a 1 to a 2 manually that should count as noload
       }
    } else {
        plan_counter[6] = 0;
    }
    ...
    if( _state.motion_plan == 1 || check_broken_wire ){
        ...
        //#######################
        plan_counter[1] = 0;
        /*if( ++plan_counter[0] > 10 ){
            // could also enforce a small retraction every x steps..
        ...
        if( _state.motion_plan == 5 ){
            // this is buggy in the new version
            // the counter resets below
            // but I will look at this later
            // not urgent
            if( ++plan_counter[4] > 50 ){
                gconf.process_error    = 2;
                gconf.edm_pause_motion = true;
                plan_counter[4] = 0;
            }
            _state.motion_plan = 4;
            ...
            if( _state.motion_plan == 3 && ++plan_counter[2] > 10 ){
                //toggle_pwm( true, 10 );
                //toggle_pwm( false, 4 );
                plan_counter[2] = 0;
            }
    ...
    //################################
    // Set/Reset short circuit timer
    // old: if( _state.motion_plan >= 4 ){
    // new: if( _state.real_short ){
    //################################
    if( _state.motion_plan >= 4 ){
    //if( _state.real_short ){
        if( short_circuit_start_time == 0 ){
            short_circuit_start_time = esp_timer_get_time();
        }
        short_circuit_estop_reset_count = 0;
    } else if ( 
        ++short_circuit_estop_reset_count > 5 
    ){
...
bool IRAM_ATTR G_EDM_PLANNER::protection_logic(){ return true; }
    ...
    if( !pre_process_history_line( line ) ){
       ...
       if( _state.real_short ){
        ...
        switch ( plan )
        {
            ...
            case 3: // upper setpoint range3...
                line.skip_feed  = true;
                line.step_delay = process_speeds.EDM;
            break;

            case 2: // bottom of setpoint range
                --no_load_steps;
                line.skip_feed  = true;
                line.step_delay = process_speeds.EDM;
            break;

            default: // below setpoint
                line.skip_feed  = false;
                line.step_delay = process_speeds.EDM;
            break;
...
bool IRAM_ATTR G_EDM_PLANNER::no_workload_finish_reached(){
    ...
    if( sys_position[sinker_axis] <= ( last_workload_at - no_workload_steps_to_finish ) ){
        //Serial.println("no workload finish reached");
        return true;
    }
    return false;
}
...
bool IRAM_ATTR G_EDM_PLANNER::do_flush_if_needed(){
    if( ui_controller.check_if_time_to_flush() ){
        ...
        retraction( sinker_axis, flush_retract_mm, flush_offset_mm, false, disable_spark_for_flushing );
        ui_controller.reset_flush_retract_timer();
        _state.total_retraction_steps = 0;
        short_circuit_start_time      = 0;
        plan_counter[4]                 = 0;
...
bool G_EDM_PLANNER::retraction( int stepper, float travel_mm, float offset_mm, bool oneway, bool disable_pwm ){
    if( disable_pwm ){
        toggle_pwm( true, 0 );
    }
    ...
    memcpy(__target,          sys_position, sizeof(sys_position));
    int max_pos             = round( DEFAULT_HOMING_PULLOFF * axis_settings[sinker_axis]->steps_per_mm->get() ) * -1;
    int __travel_steps      = round( travel_mm * axis_settings[sinker_axis]->steps_per_mm->get() );
    int __offset_steps      = round( offset_mm * axis_settings[sinker_axis]->steps_per_mm->get() );
    int start_step_position = __backup_position[sinker_axis]; // log start position for this axis
    int axis_target_step    = __target[sinker_axis] + __travel_steps;
