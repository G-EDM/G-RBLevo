...
void G_EDM_UI_CONTROLLER::loader_screen()
{
  ...
  tft.drawString("EVO II @ " + String( FIRMWARE_VERSION ), 20, 70, 2);vTaskDelay(1);
...
void G_EDM_UI_CONTROLLER::firmware_update()
{
  fill_screen(TFT_BLACK);vTaskDelay(1);
  tft.setTextSize(3);
  tft.setTextColor(TFT_OLIVE);
  ...
  tft.setTextSize(2);
  tft.setTextColor(TFT_PURPLE);
  tft.drawString("Firmware updating!", 20, 70, 2);vTaskDelay(1);
  ui_controller.filehandler->firmware_update();
}
...
void IRAM_ATTR ui_task(void *parameter){
  ...
  if( ui_controller->filehandler->has_new_firmware() ){
      ui_controller->firmware_update();vTaskDelay(1);
  }
...
bool G_EDM_UI_CONTROLLER::reset_display(){
    tft.setRotation(3);vTaskDelay(1);
    fill_screen(TFT_BLACK);vTaskDelay(1);
    tft.setCursor(0, 0, 4);vTaskDelay(1);
    touch_calibrate( false );vTaskDelay(1);
...
void G_EDM_UI_CONTROLLER::render_page_settings_mode()
{
  ...
  if (operation_mode == 1)
  {
    ...
    switch (single_axis_mode_direction)
    {
        ...
        /*switch (single_axis_mode_direction){
          case 1:
              single_axis_mode_direction = 2;
          break;
          case 2:
              single_axis_mode_direction = 1;
          break;
          case 3:
              single_axis_mode_direction = 4;
          break;
          case 4:
              single_axis_mode_direction = 3;
          break;
          default:
          break;
        }*/
...
void G_EDM_UI_CONTROLLER::draw_probing_buttons( int active )
{
  ...
  tft.drawString(String(probe_trigger_current) + "%", 230, 100, 2);vTaskDelay(1);
    ...
    else if (active_page == 7)
    {
      ...
      else if (motion_tab_active == 2)
      {
        ...
        else if (x >= 160 && x <= 320 && y >= 90 && y <= 115)
        {
          ...
          open_keyboard(probe_trigger_current, "Current (%)", "Current above this is cosnidered \ra contact.");
          ...
          probe_trigger_current = new_drop_noload;
...
void G_EDM_UI_CONTROLLER::save_settings(String file_name)
{
  ...
  settings += "15:" + String(probe_trigger_current) + ";";
  ...
  settings += "60:" + String( dpm_current_probing ) + ";";
  settings += "61:" + String( dpm_voltage_probing ) + ";";
  settings += "62:" + String( steps_per_mm_config.x ) + ";";
  settings += "63:" + String( steps_per_mm_config.y ) + ";";
  settings += "64:" + String( steps_per_mm_config.z ) + ";";
  settings += "65:" + String(probe_logic_low_trigger_treshold) + ";";
  settings += "66:" + String( adc_critical_conf.forward_block_channel_shorts_num ) + ";";
...
void G_EDM_UI_CONTROLLER::update_setting(String setting, String value)
{
  ...
  {
    probe_trigger_current = value.toFloat();
  }
  else if (setting == "16")
  {
  ...
  else if(setting == "48")
  {
    ...
    dpm_current_probing = value.toFloat();
    //set_full_range_avg_treshhold( value.toInt() );
  }
  else if(setting == "61"){
    dpm_voltage_probing = value.toFloat();
    ...
    probe_logic_low_trigger_treshold = value.toInt();
  }
  else if(setting == "66"){
    adc_critical_conf.forward_block_channel_shorts_num = value.toInt();
  }
...
void G_EDM_UI_CONTROLLER::sinker_drill_single_axis(){
    ...
    if( enable_dpm_rxtx ){
        // restore the pre probing dpm settings
...
void IRAM_ATTR G_EDM_UI_CONTROLLER::draw_in_process_settings(){
    ...
    if( in_process_page == 1 ){
        ...
        tft.drawString("vChannel shorts block", 5,130,2);vTaskDelay(1);
        tft.drawString("vChannel shorts pOff", 5,160,2);vTaskDelay(1);
    ...
    } else if( in_process_page == 11 ){
        tft.drawString("Probe Settings",      5, 10, 2);vTaskDelay(1);
        tft.drawString("DPM probe voltage",   5, 40, 2);vTaskDelay(1);
        tft.drawString("DPM probe current",   5, 70, 2);vTaskDelay(1);
        tft.drawString("Frequency",           5, 100, 2);vTaskDelay(1);
        tft.drawString("Duty",                5, 130, 2);vTaskDelay(1);
        tft.drawString("Setpoints",           5, 160, 2);vTaskDelay(1);
    }
    ...
    if( in_process_page == 1 ){
        ...
        tft.drawNumber( adc_critical_conf.voltage_feedback_treshhold, 180, 100, 2 );vTaskDelay(1);
        tft.drawNumber( adc_critical_conf.forward_block_channel_shorts_num, 180, 130, 2 );vTaskDelay(1);
        tft.drawNumber( adc_critical_conf.max_channel_shorts_for_pulse_off, 180, 160, 2 );vTaskDelay(1);
    } else if( in_process_page == 6 ){
      ...
      tft.drawString( "Start" , 239, 42, 2 );vTaskDelay(1);
        ...
        tft.setTextColor(TFT_GREEN);
        tft.drawString(String(dpm_voltage_probing)+"V",   180, 40, 2);vTaskDelay(1);
        tft.drawString(String(dpm_current_probing)+"A",   180, 70, 2);vTaskDelay(1);
        tft.drawString(convert_frequency_units(pwm_frequency_probing), 180, 100, 2);vTaskDelay(1);
        tft.drawString(String(pwm_duty_probing)+"%", 180, 130, 2);vTaskDelay(1);
        tft.drawString(String(probe_trigger_current) + "%", 140, 160, 2);vTaskDelay(1);
        tft.drawString("dig. Low: " + String(probe_logic_low_trigger_treshold), 200, 160, 2);vTaskDelay(1);
    }
            ...
            if( in_process_page == 1 ){
                ...
                if(  x >= 230 && x <= 310 && y >= 5 && y <= 35 ){
                  ...
                  continue;
                    ...
                    float tresh_percent = float(adc_critical_conf.voltage_feedback_treshhold);
                    open_keyboard( tresh_percent, "Voltage drop threshhold (ADC)", "If the voltage ADC drops below this\rit is considered a hard short circuit.");
                    float new_tresh = get_keyboard_result();
                    new_tresh = MIN( VSENSE_RESOLUTION, round( new_tresh ) );
                    ...
                    open_keyboard( float( adc_critical_conf.forward_block_channel_shorts_num ), "Channel shorts max fBlock", "Num shorts within the voltage i2s batch \rto block forward motion.");
                    int result = round( get_keyboard_result() );
                    adc_critical_conf.forward_block_channel_shorts_num = result;
                    draw_in_process_settings();
                    continue;
                } else if( x >= 0 && x <= 320 && y >= 155 && y <= 185 ){
                    open_keyboard( float( adc_critical_conf.max_channel_shorts_for_pulse_off ), "Channel shorts max pOff", "Num shorts within the voltage i2s batch \rto trigger pulse off.");
                    int result = round( get_keyboard_result() );
                    adc_critical_conf.max_channel_shorts_for_pulse_off = result;
                    draw_in_process_settings();
                    continue;
                }
              ...
              if( x >= 0 && x <= 320 && y >= 35 && y <= 65 ){
                open_keyboard( dpm_voltage_probing, "Probing voltage (V)", "Only used if DPM support\ris enabled");
                float result = get_keyboard_result();
                if( result < 0.0 ){
                  result = 0.0;
                } else if( result > 60.0 ){
                ...
                dpm_voltage_probing = result;
                draw_in_process_settings();
                continue;
              } else if( x >= 0 && x <= 320 && y >= 65 && y <= 95 ){
                open_keyboard( dpm_current_probing, "Probing current (A)", "Only used if DPM support\ris enabled");
                float result = get_keyboard_result();
                if( result < 0.0 ){
                  result = 0.0;
                } else if( result > 60.0 ){
                ...
                continue;
                  ...
                  draw_in_process_settings();
                  continue;
              } else if( x >= 200 && x <= 320 && y >= 155 && y <= 185 ){

                  open_keyboard(float(probe_logic_low_trigger_treshold), "Logic low treshhold (ADC)", "The vFd logic low treshhold\rfor probing.");
                  int new_treshold = round( get_keyboard_result() );
                  if( new_treshold < 0 ){
                    new_treshold = 0;
                  } else if( new_treshold > VSENSE_RESOLUTION ){
                    new_treshold = VSENSE_RESOLUTION;
                  }
                  probe_logic_low_trigger_treshold = new_treshold;
                  draw_in_process_settings();
                  continue;
              }
...
void IRAM_ATTR G_EDM_UI_CONTROLLER::restart_display(){
    tft.init();vTaskDelay(1);
    tft.setRotation(3);vTaskDelay(1);
    fill_screen(TFT_BLACK);vTaskDelay(1);
    tft.setCursor(0, 0, 4);vTaskDelay(1);
...
void G_EDM_UI_CONTROLLER::update_display_minimal()
{
  ...
  if (!is_probing()&&get_touch(&x, &y))
  {
    ...
    else if (x > 81 && x < 320 && y > 210 && y < 240)
    {
      ...
      restart_display(); // Sometimes the spark can mess with the display
                         // and once the display is broken there is no other way to recover it
                         // but a restart. And there is no way to check IF the display is messed up
                         // so for now the display will reset once the resume button is pressed
                         // This is ok. Button is on the right bottom and even with a blank display
                         // easy to hit. If the display goes beserk in the process just hit the right bottom on
                         // the display twice with a shrot pause in between
                         // display will go white on restarts. That sucks too.
                         // Maybe a little push button for this would be good. 
...
void draw_custom_line_maker( void ){
