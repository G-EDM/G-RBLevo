...
{
  open_keyboard(vsense_drop_range_min, "Setpoint minimum (%)", "If voltage drop is below this value the \rtool moves on. (" + String( vsense_drop_range_min ) + "% = " + String( percentage_to_mv( vsense_drop_range_min ) ) + "mV)");
...
{
  open_keyboard(vsense_drop_range_max, "Setpoint maximum (%)", "If voltage drops more then this percentage the \rtool retracts. (" + String( vsense_drop_range_max ) + "% = " + String( percentage_to_mv( vsense_drop_range_max ) ) + "mV)");
...
void G_EDM_UI_CONTROLLER::save_settings(String file_name)
{
  ...
  settings += "56:" + String( adc_critical_conf.high_plan_at ) + ";";
...
void G_EDM_UI_CONTROLLER::update_setting(String setting, String value)
{
  ...
  else if(setting == "48")
  {
    ...
    adc_critical_conf.high_plan_at = value.toInt();
  }
  else if(setting == "57"){
...
void IRAM_ATTR G_EDM_UI_CONTROLLER::draw_in_process_settings(){
    ...
    if( in_process_page == 1 ){
        ...
        tft.drawString("fAVG treshhold",    5, 100, 2);vTaskDelay(1);
    ...
    if( in_process_page == 1 ){
        ...
        if( adc_critical_conf.full_range_average_treshhold <= 0 ){
          tft.drawString( "Disabled", 180, 100, 2);vTaskDelay(1);
        } else {
            float tresh_percent = 100.0/float(VSENSE_RESOLUTION)*float(adc_critical_conf.full_range_average_treshhold);
            tft.drawString( String( tresh_percent, 2 ) + "%", 180, 100, 2);vTaskDelay(1);
        }
        ...
        String text = "";
        switch ( plconfig.early_exit_on_plan ){
          case 1:
            text = "Below Setpoint";
            break;
          break;
        ...
        tft.drawString( text, 180, 70, 2 );vTaskDelay(1);
            ...
            if( in_process_page == 1 ){
                ...
                if(  x >= 230 && x <= 310 && y >= 5 && y <= 35 ){
                  ...
                  continue;
                    ...
                    float tresh_percent = 100.0/float(VSENSE_RESOLUTION)*float(adc_critical_conf.full_range_average_treshhold);
                    open_keyboard( tresh_percent, "Full range AVG treshhold (%)", "If the full range AVG rises above this value\rsomething is triggered. 0=disabled");
                    float new_tresh = get_keyboard_result();
                    new_tresh = round( float(VSENSE_RESOLUTION)/100.0*new_tresh );
                    set_full_range_avg_treshhold( new_tresh );
                    draw_in_process_settings();
                    continue;
                ...
                if(  x >= 0 && x <= 310 && y >= 35 && y <= 65 ){
                    ...
                    if(++plconfig.early_exit_on_plan>3){
