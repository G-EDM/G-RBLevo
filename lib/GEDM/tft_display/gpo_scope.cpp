...
void IRAM_ATTR scope_task( void * parameter ){
    ...
    for(;;){
      ...
      /*if( data[2] != 0 && gpconf.enable_us ){
        __this->set_us_per_read( data[2] );
      }*/
...
IRAM_ATTR void GPO_SCOPE::draw_wave( int steps ){
    ...
    if( !gpo_data.scope_active || !gpconf.enablewave ){
        ...
        canvas_b.createSprite( gpo_data.scope_width_total-5, 20 );
    ...
    color = avg_control();
    //canvas_b.drawFastHLine( gpo_data.scope_width-50, gpo_data.scope_height-pixel_vertical, 50, color  );
    //canvas_b.drawFastHLine( 0, gpo_data.scope_height-pixel_vertical, gpo_data.scope_width, color  );
    canvas_b.drawFastHLine( 0, gpo_data.scope_height-pixel_vertical, 10, color  );
    canvas_b.drawFastHLine( 0, gpo_data.scope_height-pixel_vertical+1 , 10, color  );
    //canvas_b.drawFastVLine( 0, gpo_data.scope_height, pixel_vertical, color );
    //canvas_b.drawFastVLine( 1, gpo_data.scope_height, pixel_vertical, color );
    draw_scope_meta();//vTaskDelay(1);
    canvas_b.pushSprite( gpo_data.scope_pos_x, gpo_data.scope_pos_y+20 );vTaskDelay(1);
    canvas_b.deleteSprite();
...
IRAM_ATTR void GPO_SCOPE::draw_scope_meta(){
        ...
        if( gpo_data.show_values ){
            //canvas_b.drawFloat( gpo_data.scope_ratio, 1, 0, 2,  2 );
            int pos_start = 5;
            if( gpconf.enable_plan ){
                canvas_b.drawNumber( motion_plan, pos_start, 2,  2 );
            }
            if( gpconf.enable_ksps ){
                pos_start += 43;
                canvas_b.drawNumber( gpo_data.sample_rate, pos_start, 2, 2 );
            }
            /*if( gpconf.enable_mv ){
                pos_start += 43;
                canvas_b.drawNumber( adc_to_voltage( gpo_data.last_adc_value ), pos_start, 2,  2 );
            }*/
            /*if( gpconf.enable_us ){
                pos_start += 43;
                canvas_b.drawNumber( gpo_data.us_per_read, pos_start, 2,  2 );
            }*/
            if( gpconf.enable_line ){
                pos_start += 43;
...
IRAM_ATTR void GPO_SCOPE::draw_setpoint(){
    ...
    Serial.println( String( gpo_data.scope_height ) );
    Serial.println( String( gpo_data.resolution ) );
    Serial.println( String( gpo_data.setpoint_min ) );
    Serial.println( String( gpo_data.setpoint_max ) );
    Serial.println( "Debug end ############################" );*/
    canvas_b.deleteSprite();
    canvas_b.createSprite( 5, gpo_data.scope_height );vTaskDelay(1);
    ...
    canvas_b.fillRect( 0,  max, 5, min-max, TFT_GREEN );vTaskDelay(1);
...
IRAM_ATTR void GPO_SCOPE::draw_header(){
    ...
    //canvas_b.drawString( "X", 5, 2, 2 );vTaskDelay(1);

    if( gpconf.enable_plan ){
        canvas_b.drawString( "Plan", pos_start, 2,  2 );vTaskDelay(1);
    }
    ...
    /*if( gpconf.enable_mv ){
        pos_start += 43;
        canvas_b.drawString( "mV", pos_start, 2, 2 );vTaskDelay(1);
    }*/
    if( gpconf.enable_line ){
        pos_start += 43;
...
IRAM_ATTR void GPO_SCOPE::update_setpoint( uint32_t smin, uint32_t smax ){
    ...
    gpo_data.center_setpoint = ( gpo_data.setpoint_min + gpo_data.setpoint_max ) / 2;
    if( gpo_data.scope_active ){
        draw_setpoint();
    }
}
...
IRAM_ATTR void GPO_SCOPE::set_full_range_avg_treshhold( int tresh ){
    gpo_data.full_range_tresh = tresh;
}