...
IRAM_ATTR int G_EDM_SENSORS::get_motion_plan( volatile adc_readings &adc_data ){
    ...
    if( is_probing() ){
        ...
        if (
            ...
            plan = 2; // pause motion
            if( 
                ++counter[5] >= MOTION_PLAN_PROBE_CONFIRMATIONS 
                //|| sfeedback.pin_is_low 
            ){
    ...
    // Test: Last version triggered if above setpoint
    // or pin low. But above setpoint seems to heavily
    // switch here... 
    // if( edm_process_is_running && ( realtime_plan == 4 || sfeedback.pin_is_low ) ){
    // new: only turn it off on pin_low
    //##############################################
    if( 
