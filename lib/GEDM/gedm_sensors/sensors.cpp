...
void IRAM_ATTR gpio_35_on_interrupt( void ){
  //if( pconf.pwm_is_off ){ return; } // don't overwrite last state while pwm is off
  uint32_t state = (gpio_input_get_high()>>(DIGITAL_INPUT_FEED_HOLD - 32))&BIT0;
  if( state == 1 ){
    sfeedback.pin_is_low = false;
    if( !block_short_low_override) { sfeedback.low_since = 0; }
    //sfeedback.low_since  = 0;
  } else {
    sfeedback.pin_is_low = true;
    if( sfeedback.low_since == 0 ){
        sfeedback.low_since  = esp_timer_get_time();
    }
  }
}
...
void IRAM_ATTR G_EDM_SENSORS::reset_sensor(){
    ...
    adc_wave.last_rising_edge_adc      = setpoints.upper; // dirty solution
...
void IRAM_ATTR G_EDM_SENSORS::generate_setpoint_min_max_adc( int adc_reading_reference ){
    ...
    float short_circuit_setpoint = vsense_drop_range_max+5.0;
    float short_circuit_break_setpoint = vsense_drop_range_max+20.0;
    if( short_circuit_setpoint > VSENSE_DROP_RANGE_SHORT ){
        short_circuit_setpoint = VSENSE_DROP_RANGE_SHORT;
    }
    if( short_circuit_break_setpoint > VSENSE_DROP_RANGE_SHORT ){
        short_circuit_break_setpoint = VSENSE_DROP_RANGE_SHORT;
    }
    float resolution;
    ...
    setpoints.short_circuit       = round( resolution * short_circuit_setpoint );
    setpoints.probing             = round( resolution * vSense_drop_range_noload ); 
    setpoints.short_circuit_break = round( resolution * short_circuit_break_setpoint ); 
    setpoints.center = round( ( setpoints.lower + setpoints.upper ) / 2 );
    setpoints.center = round( ( setpoints.center + setpoints.upper ) / 2 );
...
//######################################################
IRAM_ATTR void set_signals( volatile adc_readings &adc_data, int &realtime_plan ){
    if( adc_data.recent <= adc_critical_conf.zero_treshhold ){
        if( ++adc_data.zeros_in_a_row > adc_critical_conf.zeros_jitter ){
            adc_data.highs_in_a_row = 0;
        }
    } else {
        if( realtime_plan >= adc_critical_conf.high_plan_at ){ // default >=4/above setpoint
...
IRAM_ATTR void adjust_plan_to_signals( volatile adc_readings &adc_data, int &plan, bool &create_negative_plan ){
    if( use_stop_and_go_flags ){
        if( plan == 1 && adc_data.zeros_in_a_row <= adc_critical_conf.zeros_in_a_row_max ){
            ...
            plan = MAX( plan, 4 );//new 4..
        }
    }
}

IRAM_ATTR void adjust_plan_to_edge( volatile adc_readings &adc_data, int &plan ){
...
IRAM_ATTR void do_retract_soft( volatile adc_readings &adc_data, int &plan, int &full_range_plan, int &realtime_plan ){
    ...
    if( 
        adc_data.avg_full_range > adc_critical_conf.full_range_average_treshhold 
    ){ plan = MAX( plan, 3 ); }
...
IRAM_ATTR void do_retract_hard( int &plan, int &full_range_plan, int &realtime_plan ){
    if( 
        ...
        //|| plan            >= 4
        //|| full_range_plan >= 4
        // realtime_plan is basically useless here
    ){ plan = 5; }
}
...
IRAM_ATTR void do_forward_motion( 
    ...
    if( 
        ...
        && plan            == 1
        && realtime_plan   == 1
        //&& adc_data.timestamp > adc_data.rising_edge_ignition_delay // 
        //&& adc_data.avg_full_range < adc_critical_conf.full_range_average_treshhold
        //&& !spark_counter.pulse_block
    ){
...
IRAM_ATTR int G_EDM_SENSORS::get_motion_plan( volatile adc_readings &adc_data ){
    ...
    //##############################################
    // Toggle PWM off on shorts and above setpoint
    //##############################################
    if( edm_process_is_running && ( realtime_plan == 4 || sfeedback.pin_is_low ) ){
    ...
    set_signals( adc_data, realtime_plan );
    adjust_plan_to_edge( adc_data, plan );
...
//######################################################################
// 1 = is below setpoint -> move
// 2 = is within setpoint range below center 
// 3 = is within setpoint range above center
// 4 = is above setpoint
// 5 = pretty sure shorted ( this is not fully done yet )
//######################################################################
IRAM_ATTR int G_EDM_SENSORS::get_raw_plan( int adc_value ){
    ...
    if( adc_value < setpoints.lower ){
        ...
        plan = 4; // above setpoint change to 3?
    } else if( adc_value > setpoints.center ){
...
int IRAM_ATTR G_EDM_SENSORS::get_calculated_motion_plan(){
    int plan = 0;
    if( motion_plan == 5 && sfeedback.low_since != 0 ){
        // ongoing short circuit
        return 5; // bypass the queue etc. and go straight for a short correction
    }
