...
bool IRAM_ATTR G_EDM_SENSORS::adc_to_scope( bool recent ){
    ...
    data_stats[4] = adc_data.voltage_channel_adc_recent;//sfeedback.pin_is_low ? 0 : 1;
    // add the adc reading 
...
void IRAM_ATTR vsense_task_listener(void *parameter){ 
    ...
    for(;;){
        ...
        if( ++i2s_data.num_samples_in_pool <= i2sconf.I2S_pool_size ){
            ...
            if( i2s_data.highest_adc < setpoints.lower && !sfeedback.pin_is_low ){ // only collect more if forward motion is indicated
                continue;
            }
        } 
...
IRAM_ATTR void do_retract_hard( int &plan, int &full_range_plan, int &realtime_plan ){
    if( 
        ...
        plan = 5; 
        sfeedback.had_short_condition = 1;
        counter[0] = 0;
...
IRAM_ATTR void read_voltage_feedback( volatile adc_readings &adc_data ){
    ...
    if( adc_data.voltage_channel_adc_recent < adc_critical_conf.voltage_feedback_treshhold ){
        sfeedback.pin_is_low = true;
        sfeedback.reset_count = 3;
        if( ++sfeedback.count > 100000 ){ sfeedback.count = 100000; };
    } else {
...
IRAM_ATTR void adjust_plan_to_edge( volatile adc_readings &adc_data, int &plan, bool &create_negative_plan ){
    if( adc_data.edge_type > 0 ){ // rising edge situation
        if( adc_data.edge_type == 1 ){ // rising edge from bottom
            ...
            if( plan == 1 ){ create_negative_plan = true; }
            plan = MAX( adc_data.edge_type==2?3:5, plan );
        }
    }
...
IRAM_ATTR int G_EDM_SENSORS::get_motion_plan( volatile adc_readings &adc_data ){
    ...
    if( plan < 4 ){
        ...
        ++plan;
        pwm_off    = true;
        counter[7] = adc_critical_conf.retract_confirmations;
    }

    if( 
        adc_data.edge_type == 3 
        && plan >= 4 
    ){
        pwm_off    = true;
        counter[7] = adc_critical_conf.retract_confirmations;
    ...
    pwm_off_protection( pwm_off );


    if( sfeedback.had_short_condition == 1 ){
        if( plan < 5 ){
            ++counter[0];
        }
...
IRAM_ATTR void G_EDM_SENSORS::pwm_off_protection( bool enforce = false ){
    if( 
        ...
        && ( enforce || ( sfeedback.pin_is_low && adc_critical_conf.pulse_off_duration>0 && sfeedback.count > adc_critical_conf.pwm_off_count ))
    ){ 
