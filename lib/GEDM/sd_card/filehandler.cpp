...
bool G_FILEHANDLER::has_new_firmware(){
  if( !has_sd_card ){ return false; } 
  if( get_file_exists("/firmware.bin") ){
      return true;
  } return false;
}


bool G_FILEHANDLER::firmware_update(){

  if( !has_sd_card ){ return false; } 
  /** check for new firmware **/
  if( get_file_exists("/firmware.bin") ){
    // firmware file found
    int error = open_file( "", "firmware.bin" );
    if( error == 0 ){
      // success
