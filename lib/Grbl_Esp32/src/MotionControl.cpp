...
void __attribute__((weak)) IRAM_ATTR motors_to_cartesian(float* cartesian, float* motors, int n_axis) {
...
void mc_arc(float*            target,
    ...
    //memset( previous_position, 0, sizeof(previous_position) );
    ...
    if (segments) {
        ...
        for (i = 1; i < segments; i++) {                  // Increment (segments-1).
            ...
            glimits.limitsCheckSoft(position);
    ...
    glimits.limitsCheckSoft(target);
...
void mc_homing_cycle(uint8_t cycle_mask) {
    ...
    if ( grbl.user_defined_homing( cycle_mask ) ) {
        return;
    }
    ...
    //glimits.limits_disable();  // Disable hard limits pin change register for cycle duration
    // -------------------------------------------------------------------------------------
    ...
    */
    if (cycle_mask) {
        if (!axis_is_squared(cycle_mask)) {
            glimits.limits_go_home(cycle_mask);  // Homing cycle 0
        } else {
            n_homing_locate_cycle = 0;  // don't do a second touch cycle
            glimits.limits_go_home(cycle_mask);
        ...
        for (int cycle = 0; cycle < MAX_N_AXIS; cycle++) {
            ...
            if (homing_mask) {  // if there are some axes in this cycle
                ...
                if (!axis_is_squared(homing_mask)) {
                    glimits.limits_go_home(homing_mask);  // Homing cycle 0
                } else {
                    n_homing_locate_cycle = 0;  // don't do a second touch cycle
                    glimits.limits_go_home(homing_mask);
                    n_homing_locate_cycle = NHomingLocateCycle;  // restore to default value
                    glimits.limits_go_home(homing_mask);
                    glimits.limits_go_home(homing_mask);
                }
    ...
    gproto.protocol_exec_rt_system();  // Check for reset and set system abort.
    if (sys.abort) {
...
GCUpdatePos mc_probe_cycle(float* target, plan_line_data_t* pl_data, uint8_t parser_flags) {
    ...
    gprobe.set_probe_direction( is_probe_away );
    // After syncing, check if probe is already triggered. If so, halt and issue alarm.
    // NOTE: This probe initialization error applies to all probing cycles.
    if ( gprobe.probe_get_state() ^ is_probe_away ) {  // Check probe pin state.
    ...
    gproto.protocol_exec_rt_system();   // Check and execute run-time commands
...
void mc_reset() {
    if (!sys_rt_exec_state.bit.reset) {
        ...
        if (
            (sys.state == State::Cycle || sys.state == State::Homing || sys.state == State::Jog)
        ) {
...
Error jog_execute(plan_line_data_t* pl_data, parser_block_t* gc_block, bool* cancelledInflight) {
    ...
    #endif
    if (soft_limits->get()) {
        if ( glimits.limitsCheckTravel(gc_block->values.xyz) ) {
