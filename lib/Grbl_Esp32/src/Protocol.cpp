...
IRAM_ATTR void protocol_main_loop(void *parameter){ 
    ...
    for (;;) {
        ...
        if (SD_ready_next) {
            ...
            char gcode_line_sd[255];
            if (filehandler.get_line( gcode_line_sd, 255 )) {
                SD_ready_next = false;
                report_status_message( __this->execute_line(gcode_line_sd) );
            } else {
        ...
        if( timestamp_us - idle_timer > 1000000 ){
