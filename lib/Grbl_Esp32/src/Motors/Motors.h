...
class G_STEPPER_MANAGER{
	...
	public:
	    G_STEPPER_MANAGER();
		void set_steps_per_mm( int steps, int axis );
        void init( void );
        ...
        bool IRAM_ATTR motors_direction(uint8_t dir_mask,int ignore_axis=-1);
        void IRAM_ATTR motors_step(uint8_t step_mask,int ignore_axis=-1);
